// Repris de swift-psl — https://github.com/ameshkov/swift-psl
// Copyright 2025 Andrey Meshkov. Sous licence MIT : voir LICENSE dans ce dossier.
//
// **Recopié plutôt que dépendu, pour une raison de structure et non de goût.** SwiftPM
// range les ressources d'une dépendance dans un paquet posé *à la racine* de `Wuji.app`,
// à côté de `Contents`, et le code qu'il engendre ne sait le chercher que là. macOS exige
// l'inverse : tout doit vivre sous `Contents`, sinon l'application ne peut pas être signée
// — mesuré, « unsealed contents present in the bundle root », et donc pas de notarisation,
// donc pas de distribution possible.
//
// Le seul écart avec l'original est la façon de trouver les données : voir `Data.swift`.
...
public enum PublicSuffixList {
    ...
    nonisolated(unsafe) private static let commonTrie: ByteArraySuffixTrie = {
        guard
            let url = PublicSuffixData.url(of: "common"),
    ...
    nonisolated(unsafe) private static let negatedTrie: ByteArraySuffixTrie = {
        guard
            let url = PublicSuffixData.url(of: "negated"),
            let data = try? Data(contentsOf: url)
        else {
    ...
    nonisolated(unsafe) private static let asteriskTrie: ByteArraySuffixTrie = {
        guard
            let url = PublicSuffixData.url(of: "asterisk"),
            let data = try? Data(contentsOf: url)
        else {
